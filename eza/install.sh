...
function get_current_version() {
	...
	if command -v eza >/dev/null; then
		version=$(eza --version | grep -o 'v[0-9]*\.[0-9]*\.[0-9]*')
		version=${version#v}
	fi
	echo "${version}"
}
...
function main() {
	...
	if ${FORCE}; then
		echo "Force mode is enabled. Installing eza ${latest_version} regardless of the current version."
	else
		compare_versions_and_exit "${current_version}" "${latest_version}"
		if [ "${current_version}" = "0.0.0" ]; then
			echo "Installing eza ${latest_version}."
		else
			echo "Upgrading eza ${current_version} -> ${latest_version}"
		fi
	fi
	...
	if [ "${os}" = "darwin" ]; then
		...
		file="eza_${arch}-unknown-linux-gnu"
		output_file="${tmp_dir}/eza.tar.gz"
		...
		copy_and_set_permissions "${tmp_dir}/eza"
