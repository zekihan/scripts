...
function get_current_version() {
	...
	if command -v yq >/dev/null; then
		version=$(yq --version)
		# yq (https://github.com/mikefarah/yq/) version v4.41.1
		version=${version##*v}
	fi
	echo "${version}"
}
...
function main() {
	...
	if ${FORCE}; then
		echo "Force mode is enabled. Installing yq ${latest_version} regardless of the current version."
	else
		compare_versions_and_exit "${current_version}" "${latest_version}"
		if [ "${current_version}" = "0.0.0" ]; then
			echo "Installing yq ${latest_version}."
		else
			echo "Upgrading yq ${current_version} -> ${latest_version}"
		fi
	fi
