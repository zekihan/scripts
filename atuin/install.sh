...
function main() {
	...
	if ${FORCE}; then
		echo "Force mode is enabled. Installing atuin ${latest_version} regardless of the current version."
	else
		compare_versions_and_exit "${current_version}" "${latest_version}"
		if [ "${current_version}" = "0.0.0" ]; then
			echo "Installing atuin ${latest_version}."
		else
			echo "Upgrading atuin ${current_version} -> ${latest_version}"
		fi
	fi
